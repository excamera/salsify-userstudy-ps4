...
for q,m,s in zip(qualities, mean, std):
    ...
    for p_,m_,s_,d,color in zip(padding, m, s,[100,300,550,1050],['#4c72b0', '#55a868', '#c44e52', '#8172b2']):
        ...
        if count == 0:
            if d == 1050:
                plt.text(c+p_-.10, m_-s_-0.3,str(d), fontsize=15,color=color)
            elif d == 300:
                plt.text(c+p_-.16, m_+s_+0.15,str(d), fontsize=15,color=color)
            elif d == 550:
                plt.text(c+p_-.23, m_-s_-0.3,str(d), fontsize=15,color=color)                
            else:
                plt.text(c+p_-.18, m_+s_+0.15,str(d), fontsize=15,color=color)
...
c = 5.25
x = 7.25
plt.plot([x,x+1.35],[c,c],lw=2,color=(0.33,0.33,0.33))
plt.plot([x,x],[c,c-.1],lw=2,color=(0.33,0.33,0.33))
plt.plot([x+1.35,x+1.35],[c,c-.1],lw=2,color=(0.33,0.33,0.33))

plt.text(x+.675, c+.175, 'Video Delay (ms)', fontsize=15, color=(0,0,0), horizontalalignment='center',)
